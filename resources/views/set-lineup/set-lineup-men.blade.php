...
@section('content')
	...
	<input v-model="searchString" placeholder="Search">
		<transition name="fade">
			<div class="alert container set-linup-noty" :save-message="saveMessage" v-bind:class="[saveStatus ? 'alert-success' : 'alert-danger']" @click="closeNoty" v-if="showNoty" v-cloak role="alert">@{{saveMessage}}</div>
		</transition>
		<div class="row">
			<div class="col-md-6">
				<h2>Available Racers</h2>
				<draggable :list="filteredAthletes" class="dragArea" :options="{group:'athletes'}" @start="onStart">
